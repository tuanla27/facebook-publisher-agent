...
to normalize host metadata and
`backend/assets/attachment-materializer.mjs` to perform all byte handling,
validation, scanning, hashing, immutable local storage, manifest creation, and
`input.json` updates. Do not save or hash attachments in the agent. If only a
preview or an unconfirmed source is exposed, pause and ask for the original
through Cursor's supported file attachment control.
